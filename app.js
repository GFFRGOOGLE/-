/* ============================================================
   Aurora P2P Chat — app.js v11 (рабочие звонки)
   ============================================================ */
const DEBUG = true;
...
let room = null;
let sendText, sendFileMeta, sendFileChunk, sendFileEnd, sendTyping, sendName, sendKick, sendDelete, sendRole, sendCallSignal;
let onText, onFileMeta, onFileChunk, onFileEnd, onTyping, onName, onKick, onDelete, onRole, onCallSignal;
let cryptoKey = null;
...
const callState = {
  ...
  localStream: null, remoteStream: null, remotePeerId: null,
  startTime: null, timerInterval: null,
  micEnabled: true, camEnabled: true, facingMode: 'user'
};
...
(function initMenu(){
  ...
  qm.innerHTML = `
    ...
    <button id="qmSettings" style="display:flex;align-items:center;gap:10px;width:100%;padding:13px 16px;border:none;background:transparent;color:var(--text);font-size:14.5px;font-family:inherit;cursor:pointer;border-radius:10px;text-align:left;font-weight:500">⚙️ Настройки</button>
    <div style="height:1px;background:var(--border);margin:6px 4px"></div>
  ...
  $('qmClear').onclick = () => {
    closeMenu();
    if(!confirm('Очистить всю переписку?')) return;
  ...
  $('qmSettings').onclick = () => { closeMenu(); $('settingsBtn').click(); };
...
$('settingsSaveBtn').onclick = async () => {
  ...
  if(room && sendName) sendName(settings.nickname);
  showToast('✅ Сохранено');
...
/* ============================================================
   ВВОД КОМНАТЫ НА СТАРТОВОМ ЭКРАНЕ
   ============================================================ */
(function initWelcomeInput(){
  const wr = $('welcomeRoom');
  const randomBtn = $('welcomeRandom');
  const joinBtn = $('welcomeJoin');
  const roomInput = $('roomInput');
  const mainJoin = $('joinRoomBtn');
  const mainRandom = $('generateRoomBtn');
  if(!wr || !roomInput || !mainJoin) return;

  wr.value = roomInput.value || localStorage.getItem('aurora_room') || '';
  wr.addEventListener('input', () => { roomInput.value = wr.value; });

  if(randomBtn) randomBtn.onclick = () => { if(mainRandom) mainRandom.click(); wr.value = roomInput.value; };
  if(joinBtn) joinBtn.onclick = () => { roomInput.value = wr.value; mainJoin.click(); };
  wr.addEventListener('keydown', e => { if(e.key === 'Enter'){ e.preventDefault(); if(joinBtn) joinBtn.click(); } });
})();
...
$('generateRoomBtn').onclick = () => {
  ...
  const r = `${w[Math.floor(Math.random()*w.length)]}-${w[Math.floor(Math.random()*w.length)]}-${Math.floor(Math.random()*9000)+1000}`;
  $('roomInput').value = r;
  if($('welcomeRoom')) $('welcomeRoom').value = r;
};
$('joinRoomBtn').onclick = () => {
...
function kickPeer(id){
  ...
  blocked.add(id); peers.delete(id); adminIds.delete(id); peerJoinedAt.delete(id);
  renderPeers();
...
async function joinRoomById(roomId){
  ...
  try{
    ...
    // === МЕДИАПОТОК (для звонков) — нативный API Trystero ===
    if(typeof room.onPeerStream === 'function'){
      room.onPeerStream((stream, peerId) => {
        log('📹 Получен медиапоток от', peerId);
        if(blocked.has(peerId)) return;
        callState.remoteStream = stream;
        const rv = $('remoteVideo');
        rv.srcObject = stream;
        // Принудительный запуск воспроизведения (важно для мобильных)
        rv.play().catch(e => logErr('Remote play:', e));
...
/* ============================================================
   ЗВОНКИ (рабочая версия через room.addStream)
   ============================================================ */
...
async function startCall(pid, isV){
  ...
  Object.assign(callState, {
    isVideo: isV, outgoing: true, active: true, remotePeerId: pid,
    micEnabled: true, camEnabled: isV, facingMode: 'user'
  });
  ...
  try{
    ...
    lv.srcObject = s;
    lv.muted = true;
    lv.play().catch(e => logErr('Local play:', e));

    // Отправляем поток через нативный API Trystero
    if(typeof room.addStream === 'function'){
      room.addStream(s);
      log('📤 Локальный поток отправлен');
    } else {
      logErr('room.addStream недоступен');
    }

    sendCallSignal({type:'offer', isVideo:isV}, pid);
  }catch(e){
    logErr('startCall:', e);
    showToast('Нет доступа к ' + (isV ? 'камере' : 'микрофону'));
    resetCall();
  }
}
...
$('acceptCallBtn').onclick = async () => {
  ...
  $('incomingScreen').classList.remove('active');
  stopRing();
  Object.assign(callState, {incoming:false, active:true, micEnabled:true, camEnabled:callState.isVideo, facingMode:'user'});
  ...
  try{
    ...
    if(typeof room.addStream === 'function'){
      room.addStream(s);
      log('📤 Локальный поток отправлен (accept)');
    }
    ...
    logErr('accept:', e);
    sendCallSignal({type:'reject', reason:'no-media'}, pid);
    resetCall();
  }
};
...
function openCallUI(){
  ...
  $('ctrlFlip').style.display = callState.isVideo ? 'flex' : 'none';
  $('ctrlMic').classList.remove('off');
  $('ctrlCam').classList.remove('off');
  $('callAvatarStatus').textContent = 'Соединение...';

  // Если удалённый поток уже пришёл — показываем
  if(callState.remoteStream){
    const rv = $('remoteVideo');
    rv.srcObject = callState.remoteStream;
    rv.play().catch(e => logErr('Remote play (UI):', e));
  }
}
...
$('ctrlFlip').onclick = async () => {
  if(!callState.localStream || !callState.isVideo) return;
  callState.facingMode = callState.facingMode === 'user' ? 'environment' : 'user';
  try{
    const old = callState.localStream;
    const vTrack = old.getVideoTracks()[0];
    const newStream = await navigator.mediaDevices.getUserMedia({
      audio:false,
      video:{ facingMode: callState.facingMode, width:{ideal:1280}, height:{ideal:720} }
    });
    const newVTrack = newStream.getVideoTracks()[0];
    old.removeTrack(vTrack);
    vTrack.stop();
    old.addTrack(newVTrack);
    $('localVideo').srcObject = old;
    if(typeof room.replaceTrack === 'function'){
      try{ room.replaceTrack(vTrack, newVTrack); }catch(e){ logErr('replaceTrack:', e); }
    }
  }catch(e){ logErr('flip:', e); }
};
...
function resetCall(){
  ...
  // Убираем поток из соединения (Trystero умеет сам)
  try{
    if(room && typeof room.removeStream === 'function' && callState.localStream){
      room.removeStream(callState.localStream);
    }
  }catch(e){}
